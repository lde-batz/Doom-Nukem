...
/*   Updated: 2019/05/08 02:23:28 by cababou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	t_el_ui		*ui;
	void		(*ui_callback)(t_doom *doom,
		struct s_button_element *b, SDL_MouseButtonEvent event);
...
void				wfc_floor_draw(t_raycasting *rc, t_player *p,
						Uint32 **canvas);
void				wfc_ceiling_draw(t_raycasting *rc, t_player *p,
						Uint32 **canvas);
void				draw_sprites(t_doom *doom, t_raycasting *rc, t_player *p,
						double **z_buffer);
void				sprite_flat_init(t_raycasting *rc, t_player *p,
						t_sprite *sprite);
...
int					check_wall(t_doom *doom, t_list *sprites,
						t_sprite *s, t_xy *d);
void				collision_sprites(t_doom *doom, t_sprite *sprite,
						t_xy *dest);
...
void				instant_text(t_doom *d, SDL_Surface *s, char *st,
						SDL_Rect poscols);
t_el_text			*create_text(t_doom *doom, char *string, char *font_path,
						int size);
void				text_prepare(t_doom *doom, t_el_text *text, int mk_size,
						int cent);
void				text_render(t_doom *doom, SDL_Surface *surface,
						t_el_text *text);
...
void				ed_bt_edit_click(t_doom *doom, t_el_button *b,
...
void				switch_tool(t_doom *doom, int to_tool,
						t_block_type *block);
void				switch_tool_sprite(t_doom *doom, int to_tool,
						t_sprite_type *sp);

t_block_type		*make_block_type(t_doom *doom, char *bn, Uint32 bc, int bt);
t_block_type		*block_type(t_doom *d, int bt);
t_sprite_type		*make_sprite_type(t_doom *doom, char *bn, Uint32 bc,
						int bt);

void				select_block_type(t_doom *d, t_block_type *type);
void				copy_block_type(t_doom *d, t_block_type *type,
						t_mblock *blk);
void				copy_block(t_mblock *dest, t_mblock *src, int free2,
						int cpcrds);
...
void				ed_save_file(t_doom *d, t_el_button *b,
						SDL_MouseButtonEvent ev);
void				ed_test_map(t_doom *d, t_el_button *b,
						SDL_MouseButtonEvent ev);
